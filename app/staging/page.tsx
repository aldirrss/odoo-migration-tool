...
export default function StagingPage() {
  ...
  const statsQuery = useQuery({
    queryKey: ["staging-stats", activeJobId],
    enabled: !!activeJobId,
    queryFn: async () => {
      const r = await fetch(`/api/staging/stats?jobId=${activeJobId}`);
  ...
  if (!activeJobId) {
    return (
      <div className="space-y-6">
        ...
        <ProgressStepper current="clean" completed={["connections", "extract"]} />
        <Card>
