...
export default function ValidatePage() {
  ...
  const summaryQuery = useQuery({
    queryKey: ["validation-summary", activeJobId],
    enabled: !!activeJobId,
    queryFn: async () => {
      const r = await fetch(`/api/validate/summary?jobId=${activeJobId}`);
  ...
  if (!activeJobId) {
    return (
      <div className="space-y-6">
        ...
        <ProgressStepper current="validate" completed={["connections", "extract", "clean"]} />
        <Card>
